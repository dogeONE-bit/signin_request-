...
if __name__ == "__main__":
    ...
    response1 = requests.post('https://glados.rocks/api/user/checkin', cookies=cookies1, headers=headers)
    response2 = requests.post('https://glados.rocks/api/user/checkin', cookies=cookies2, headers=headers)
    response3 = requests.post('https://glados.rocks/api/user/checkin', cookies=cookies3, headers=headers)
    cookie_str4 = os.environ.get("COOKIE4", None)
    cookies4 = ParseCookiestr(cookie_str4)
    response4 = requests.post('https://glados.rocks/api/user/checkin', cookies=cookies4, headers=headers)
